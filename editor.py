...
Restarting the app resets everything to the default dataset, so be sure to save and download the in-memory data to preserve your edits. To confirm your saved file matches what is in memory, use the verification step by loading your personalised JSON file."""
    ...
    if working:
        ...
        with tab3:
            render_seasons_summary_table(working) # Top
            st.markdown("---")
            render_reference_points_editor_v2(working, years, current_resort_id) # Bottom
        with tab4:
            render_holidays_summary_table(working) # Top
            st.markdown("---")
            render_holiday_management_v2(working, years, current_resort_id, data) # Bottom
