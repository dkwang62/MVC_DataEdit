...
def render_single_season_v2(
    ...
    # We update the working copy directly here. 
    # Because 'season' is a reference to the dict inside 'working', this update is buffered.
    if not edited_df.equals(df):
        ...
        # We trigger a rerun to refresh the UI, but the data is safely in the 'working' buffer.
        st.rerun()
    ...
    for s_idx, season in enumerate(seasons):
        with st.expander(
            ...
            for key, cat in dc.items():
                ...
                # --- VISUAL CHANGE: Use Data Editor instead of list of inputs ---
                pts_data = []
                ...
                # We use a unique key for the editor to avoid conflicts
                wk = rk(resort_id, "rp_table_edit", base_year, s_idx, key)
                edited_df = st.data_editor(
                    df_pts,
                    key=wk,
                    width="stretch",
                ...
                # Write back to working copy
                # If changed, we update the dict. This effectively buffers the change.
                new_rp_dict = dict(zip(edited_df["Room Type"], edited_df["Points"]))
                if new_rp_dict != room_points:
                    cat["room_points"] = new_rp_dict
                    # We do NOT save_data() here because that implies disk commit.
    ...
    if not base_holidays:
        ...
        for h_idx, h in enumerate(base_holidays):
            ...
            with st.expander(f"🎊 {disp_name}", expanded=False):
                ...
                wk = rk(resort_id, "holiday_points_table", base_year, h_idx)
                edited_df = st.data_editor(
                    df_pts,
                    key=wk,
                    width="stretch",
    ...
    if st.session_state.data is None:
        try:
            with open("data_v2.json", "r") as f:
                ...
                if "schema_version" in clean_data and "resorts" in clean_data:
                    st.session_state.data = clean_data
                    st.toast(f"Auto-loaded {len(clean_data.get('resorts', []))} resorts", icon="✅")
